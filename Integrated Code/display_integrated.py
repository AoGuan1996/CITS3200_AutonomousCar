...
class Display():
	...
	def menu(self):
		...
		if start:
			if maps[map_button.current]["Name"] == "map_user":
				# Enable computer vision construction of new waypoints
					# cv_waypoints = cv.generate_waypoints()
				# Rebuild scenario config as below with the waypoints generated
				print(msgHeader+"User defined map selected, building waypoints")
				print(msgHeader+"Waypoints received, rebuilding scenario_config")
				new_map = {}
				new_map["Name"] = "map_user"
				# new_map["Waypoints"] = cv_waypoints
				for map in maps:
					if map["Name"] == "map_blank": # Find the blank map image and use that as background
						new_map["Image"] = map["Image"]
						new_map["Waypoints"] = map["Waypoints"] # Placeholder so the system works without the CV generated waypoints
				scenario_config = {}
...
class Button:
	...
	def render(self, screen):
		...
		if self.maps:
			...
			if self.hover:
				map_text_render = self.font_general.render("Map:", True, (255,255,255))
				map_text_rect = map_text_render.get_rect(center=(self.x + self.w * 1.6, self.y + self.h / 3))
				screen.blit(map_text_render, map_text_rect)

				file_text_render = self.font_general.render(self.maps[self.current]["Name"], True, (255,0,255))
			...
			if self.hover:
				status_text_render = self.font_general.render("Status:", True, (255,255,255))
				...
				en_text_render = self.font_general.render("Enabled" if car["Enabled"] else "Not Enabled", True, (0,255,0) if car["Enabled"] else (255,0,0))
				en_text_rect = en_text_render.get_rect(center=(self.x + self.w *2.5, self.y + self.h / 4))
				screen.blit(en_text_render, en_text_rect)

				strat_text_render = self.font_general.render("Strategy File:", True, (255,255,255))
				...
				file_text_render = self.font_general.render(car["Strategy"].split("/")[-1], True, (0,0,255))
		...
		if (self.text != ""):
			if self.cars and self.cars[self.current]["Colour"] == "white":
				...
				colour = (255, 255, 255)
			text_render = self.font_general.render(self.text, True, colour)
